...
const DiscGolfApp = () => {
  ...
  const [currentUser, setCurrentUser] = useState(null);import React, { useState, useEffect } from 'react';
import { Trophy, User, LogOut, ChevronRight, ChevronLeft, Edit, Clock, MapPin, Calendar, Plus, Minus, Check, Moon, Sun } from 'lucide-react';

const SHEET_ID = '1bzJdaMrV7sInlNtMP81hKST8-TTq2UTDujkk68w3IPU', API_KEY = 'AIzaSyBzu0SSydX4hR8eHIjo3yeg_eHL_FJhRKI';
...
const PRIMARY = '#006400', ACCENT = '#228B22';

const useDarkMode = () => {
  const [d, setD] = useState(() => localStorage.getItem('darkMode') ? JSON.parse(localStorage.getItem('darkMode')) : window.matchMedia('(prefers-color-scheme: dark)').matches);
  useEffect(() => { localStorage.setItem('darkMode', JSON.stringify(d)); document.body.classList.toggle('dark-mode', d); }, [d]);
  useEffect(() => {
    const s = document.createElement('style');
    s.innerHTML = `* {transition: background 0.3s, color 0.3s, border-color 0.3s;} body.dark-mode {background: #111827; color: #f9fafb;} .dark-mode .bg-white {background: #1f2937 !important;} .dark-mode .bg-gray-50 {background: #111827 !important;} .dark-mode .bg-gray-100 {background: #374151 !important;} .dark-mode .text-gray-900 {color: #f9fafb !important;} .dark-mode .text-gray-700 {color: #d1d5db !important;} .dark-mode .text-gray-600 {color: #9ca3af !important;} .dark-mode .text-gray-500 {color: #6b7280 !important;} .dark-mode .border-gray-200 {border-color: #374151 !important;} .dark-mode select, .dark-mode input {background: #374151 !important; color: #f9fafb !important; border-color: #4b5563 !important;} .dark-mode .bg-red-50 {background: rgba(153,27,27,0.2) !important;} .dark-mode .text-red-800 {color: #fca5a5 !important;} .dark-mode .bg-orange-50 {background: rgba(154,52,18,0.2) !important;} .dark-mode .text-orange-800 {color: #fdba74 !important;} .dark-mode .bg-blue-50 {background: rgba(30,58,138,0.3) !important;} .dark-mode .shadow-lg {box-shadow: 0 10px 15px -3px rgba(0,0,0,0.3) !important;}`;
...
const Modal = ({open, close, children, dark}) => open ? <div className="fixed inset-0 bg-black/50 flex items-end z-50" onClick={close}><div className={`${dark ? 'bg-gray-800' : 'bg-white'} w-full rounded-t-3xl p-6 max-w-md mx-auto`} onClick={e => e.stopPropagation()}>{children}</div></div> : null;

const ChangePinModal = ({open, close, submit, dark}) => {
  const [p, setP] = useState(''), [c, setC] = useState(''), [e, setE] = useState('');
  const handle = () => { if (p.length !== 4 || c.length !== 4) return setE('PIN must be 4 digits'); if (p !== c) return setE('PINs do not match'); submit(p); setP(''); setC(''); setE(''); };
  return <Modal open={open} close={close} dark={dark}><h3 className="text-xl font-bold text-gray-900 mb-4">Change PIN</h3><div className="space-y-4"><div><label className="block text-sm font-semibold text-gray-700 mb-2">New PIN</label><input type="password" maxLength="4" placeholder="4 digits" value={p} onChange={ev => setP(ev.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" /></div><div><label className="block text-sm font-semibold text-gray-700 mb-2">Confirm PIN</label><input type="password" maxLength="4" placeholder="4 digits" value={c} onChange={ev => setC(ev.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" /></div>{e && <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r"><p className="text-sm text-red-800">{e}</p></div>}<div className="flex gap-3"><button onClick={close} className="flex-1 bg-gray-200 text-gray-700 py-3.5 rounded-xl font-semibold">Cancel</button><button onClick={handle} className="flex-1 bg-blue-600 text-white py-3.5 rounded-xl font-semibold">Update</button></div></div></Modal>;
};

const StartHoleModal = ({open, close, confirm, hole, setHole, dark}) => (
  <Modal open={open} close={close} dark={dark}><h3 className="text-xl font-bold text-gray-900 mb-4">Select Starting Hole</h3><select value={hole} onChange={e => setHole(parseInt(e.target.value))} className="w-full bg-gray-50 border rounded-xl px-4 py-3 mb-4 focus:ring-2">{[...Array(18)].map((_, i) => <option key={i+1} value={i+1}>Hole {i+1}</option>)}</select><div className="flex gap-3"><button onClick={close} className="flex-1 bg-gray-200 py-3.5 rounded-xl font-semibold">Cancel</button><button onClick={confirm} className="flex-1 bg-blue-600 text-white py-3.5 rounded-xl font-semibold">Start</button></div></Modal>
);

export default function App() {
  const [view, setView] = useState('login'), [user, setUser] = useState(null), [players, setPlayers] = useState([]), [courses, setCourses] = useState([]);
  const [matches, setMatches] = useState([]), [match, setMatch] = useState(null), [scores, setScores] = useState([]), [hole, setHole] = useState(0);
  const [startHole, setStartHole] = useState(1), [online, setOnline] = useState(navigator.onLine), [pending, setPending] = useState([]);
  const [loading, setLoading] = useState(false), [error, setError] = useState(''), [showStart, setShowStart] = useState(false), [showPin, setShowPin] = useState(false);
  const [filter, setFilter] = useState('user'), [filterDate, setFilterDate] = useState(''), [filterPlayer, setFilterPlayer] = useState('');
  ...
  const loadData = async () => {
    try {
      ...
      const c = d.valueRanges[1].values.slice(1).map(r => ({id: r[0], name: r[1], pars: JSON.parse(r[4]||'{}')}));
    ...
    } catch (e) { const s = localStorage.getItem('data'); if (s) { const d = JSON.parse(s); setPlayers(d.players||[]); setCourses(d.courses||[]); setMatches(d.matches||[]); } }
  };

  const saveProgress = () => { try { localStorage.setItem(`progress-${match.id}`, JSON.stringify({scores, hole, startHole, ts: Date.now()})); } catch {} };
  const loadProgress = id => { try { const s = localStorage.getItem(`progress-${id}`); if (s) { const p = JSON.parse(s); setScores(p.scores); setHole(p.hole); setStartHole(p.startHole); return true; } } catch {} return false; };

  const processPending = async () => { try { const s = localStorage.getItem('pending'); if (s) { const u = JSON.parse(s); for (const up of u) await submit(up.matchId, up.scores, up.winner); localStorage.removeItem('pending'); setPending([]); } } catch {} };
  ...
  const submit = async (id, sc, win) => {
    ...
    try { const up = matches.map(m => m.id === id ? {...m, scoresJson: sc, winner: win, status: 'Completed'} : m); setMatches(up); localStorage.setItem('data', JSON.stringify({players, courses, matches: up})); await fetch(SCRIPT_URL, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({matchId: id, scores: sc, winner: win}), mode: 'no-cors'}); } catch {}
  };

  const login = (name, pin) => { const p = players.find(p => p.name === name && p.pin === pin); if (p) { setUser(p); setView('matches'); setError(''); } else setError('Invalid'); };
  const logout = () => { setUser(null); setView('login'); setMatch(null); setScores([]); };
  const changePin = pin => { const up = players.map(p => p.id === user.id ? {...p, pin} : p); setPlayers(up); setUser({...user, pin}); localStorage.setItem('data', JSON.stringify({players: up, courses, matches})); setShowPin(false); };

  const start = async m => { setMatch(m); const has = await loadProgress(m.id); if (!has) { const c = courses.find(c => c.name === m.venue); const init = [...Array(18)].map((_, i) => ({p1: c?.pars[i+1]||3, p2: c?.pars[i+1]||3, scored: false})); setScores(init); setHole(0); setShowStart(true); } else setView('scoring'); };
  ...
  const calcStatus = () => { let p1=0, p2=0, played=0; scores.forEach(s => { if (s.scored) { played++; if (s.p1 < s.p2) p1++; else if (s.p2 < s.p1) p2++; } }); const rem = Math.max(0, 18-played), lead = Math.abs(p1-p2); const leader = p1 > p2 ? match.player1 : p2 > p1 ? match.player2 : null; const dormie = lead > 0 && lead >= rem && played <= 18; const done = (played >= 18 && p1 !== p2) || (played > 18 && leader); const playoff = played === 18 && p1 === p2; return {p1, p2, played, lead, leader, dormie, done, playoff}; };
  ...
  const complete = async () => { const st = calcStatus(); if (!st.leader) return setError('Not complete'); setLoading(true); try { await submit(match.id, scores, st.leader); localStorage.removeItem(`progress-${match.id}`); setView('matches'); setMatch(null); setScores([]); } catch { setError('Will submit when online'); } finally { setLoading(false); } };
  const review = m => { setMatch(m); setScores(m.scoresJson||[]); setView('review'); };

  if (view === 'login') return (
    <div className="min-h-screen bg-white"><div className="max-w-md mx-auto px-4 py-8"><div className="absolute top-4 right-4"><button onClick={() => setDark(!dark)} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200">{dark ? <Sun size={20} /> : <Moon size={20} />}</button></div><div className="text-center mb-12 mt-8"><div className="inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 shadow-lg" style={{background: `linear-gradient(to bottom right, ${PRIMARY}, ${ACCENT})`}}><Trophy className="text-white" size={40} /></div><h1 className="text-3xl font-bold text-gray-900 mb-2">Matchplay</h1><p className="text-gray-500">Disc Golf Tournament</p></div>{!online && <div className="bg-orange-50 border-l-4 border-orange-400 p-4 mb-6 rounded-r"><p className="text-sm text-orange-800">Offline. Will sync.</p></div>}<form onSubmit={e => { e.preventDefault(); const f = new FormData(e.target); login(f.get('player'), f.get('pin')); }} className="space-y-4"><div><label className="block text-sm font-semibold text-gray-700 mb-2">Player</label><select name="player" required className="w-full bg-gray-50 border rounded-xl px-4 py-3 focus:ring-2"><option value="">Choose</option>{players.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}</select></div><div><label className="block text-sm font-semibold text-gray-700 mb-2">PIN</label><input type="password" name="pin" maxLength="4" placeholder="4 digits" required className="w-full bg-gray-50 border rounded-xl px-4 py-3 focus:ring-2" /></div>{error && <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r"><p className="text-sm text-red-800">{error}</p></div>}<button type="submit" className="w-full text-white py-3.5 rounded-xl font-semibold shadow-lg" style={{backgroundColor: PRIMARY}}>Sign In</button></form></div></div>
  );
  ...
  if (view === 'matches') {
    ...
    return (
      <div className="min-h-screen bg-gray-50"><div className="bg-gradient-to-br from-blue-600 to-blue-700 text-white sticky top-0 z-10 shadow-lg"><div className="max-w-md mx-auto px-4 py-6"><div className="flex justify-between items-center mb-4"><div className="flex items-center"><div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center mr-3"><User size={20} /></div><div><p className="text-sm opacity-90">Signed in as</p><p className="font-bold">{user.name}</p></div></div><div className="flex gap-2"><button onClick={() => setDark(!dark)} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg">{dark ? <Sun size={20} /> : <Moon size={20} />}</button><button onClick={() => setShowPin(true)} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg"><Edit size={20} /></button><button onClick={logout} className="p-2 bg-white/10 hover:bg-white/20 rounded-lg"><LogOut size={20} /></button></div></div>{!online && <div className="bg-white/10 px-3 py-2 rounded-lg text-sm flex items-center"><div className="w-2 h-2 bg-orange-300 rounded-full mr-2"></div>Offline • {pending.length} pending</div>}</div></div><div className="max-w-md mx-auto px-4 py-6"><div className="mb-6"><h2 className="text-xl font-bold text-gray-900 mb-4">Your Matches</h2>{upcoming.length === 0 ? (<div className="bg-white rounded-2xl shadow-sm p-8 text-center"><div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3"><Trophy className="text-gray-400" size={28} /></div><p className="text-gray-500">No upcoming matches</p></div>) : (<div className="space-y-3">{upcoming.map(m => (<div key={m.id} onClick={() => start(m)} className="bg-white rounded-2xl shadow-sm p-4 cursor-pointer hover:shadow-md"><div className="flex items-center justify-between mb-3"><div className="flex items-center text-sm text-gray-500"><Calendar size={14} className="mr-1" /><span>{m.date}</span><Clock size={14} className="ml-3 mr-1" /><span>{m.startTime}</span></div><ChevronRight className="text-blue-600" size={20} /></div><p className="font-bold text-gray-900 text-lg mb-1">{m.player1} <span className="text-gray-400 font-normal">vs</span> {m.player2}</p><div className="flex items-center text-sm text-gray-600"><MapPin size={14} className="mr-1" />{m.venue}</div></div>))}</div>)}</div><div><div className="flex items-center justify-between mb-4"><h2 className="text-xl font-bold text-gray-900">Completed</h2><select value={filter} onChange={e => {setFilter(e.target.value); setFilterDate(''); setFilterPlayer('');}} className="text-sm bg-white border rounded-lg px-3 py-1.5 focus:ring-2"><option value="user">My Matches</option><option value="all">All</option><option value="date">By Date</option><option value="player">By Player</option></select></div>{filter === 'date' && <select value={filterDate} onChange={e => setFilterDate(e.target.value)} className="w-full bg-white border rounded-lg px-4 py-2 mb-4 focus:ring-2"><option value="">Select date</option>{dates.map(d => <option key={d} value={d}>{d}</option>)}</select>}{filter === 'player' && <select value={filterPlayer} onChange={e => setFilterPlayer(e.target.value)} className="w-full bg-white border rounded-lg px-4 py-2 mb-4 focus:ring-2"><option value="">Select player</option>{plys.map(p => <option key={p} value={p}>{p}</option>)}</select>}{completed.length === 0 ? (<div className="bg-white rounded-2xl shadow-sm p-8 text-center"><p className="text-gray-500">{filter !== 'all' ? 'No matches' : 'No completed'}</p></div>) : (<div className="space-y-3">{completed.map(m => (<div key={m.id} onClick={() => review(m)} className="bg-white rounded-2xl shadow-sm p-4 cursor-pointer hover:shadow-md"><div className="flex items-center justify-between mb-2"><p className="font-bold text-gray-900">{m.player1} <span className="text-gray-400 font-normal">vs</span> {m.player2}</p><span className="text-xs text-gray-500">{m.date}</span></div><div className="flex items-center justify-between"><div className="flex items-center text-sm text-gray-600"><MapPin size={14} className="mr-1" />{m.venue}</div><div className="flex items-center"><Check size={16} className="text-green-600 mr-1" /><span className="text-sm font-semibold text-green-600">{m.winner} won</span></div></div></div>))}</div>)}</div></div><ChangePinModal open={showPin} close={() => setShowPin(false)} submit={changePin} dark={dark} /><StartHoleModal open={showStart} close={() => {setShowStart(false); cancel();}} confirm={confirmStart} hole={startHole} setHole={setStartHole} dark={dark} /></div>
    );
  }
